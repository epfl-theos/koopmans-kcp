...
!
      ...
      USE input_parameters,     ONLY : odd_nkscalfact, print_wfc_empty
...
!      REAL(DP),    ALLOCATABLE :: pink_emp(:)
      ...
      ! init wfcs
      !
      exst = reademptyc0( c0_emp, n_empx, ndr_loc )
      !
      IF( .NOT. exst ) THEN
         !
...
!                           rhor, ema0bg, desc_emp)
      ...
      ! 
      ! Linh: add a flag for print c0_emp cm_emp to iotk format
      ! Here, we allocate the a global vars to pass c0_emp and cm_emp to 
      ! They will be used to print out using iotk in cp_restart. 
      !
      IF (print_wfc_empty) THEN
         ALLOCATE (c0_emp_aux (ngw, n_empx))
         ALLOCATE (cm_emp_aux (ngw, n_empx))
         c0_emp_aux(:,:) = c0_emp(:,:)
         cm_emp_aux(:,:) = cm_emp(:,:)
      ENDIF
      ! 
      ...
      ENDIF
      !
      ! ...   Save emptystates to disk canonical orbitals
      !
      CALL writeempty( cm_emp, n_empx, ndw_loc )
      ! 
      ! ...   Save emptystates to disk minimizing orbitals
      !
      CALL writeemptyc0( c0_emp, n_empx, ndw_loc )
      !
...
!-----------------------------------------------------------------------
        ...
        fileempty = TRIM( wfc_filename( dirname, 'evctot_empty', 1 ) )
        !
        IF ( ionode ) THEN
...
!-----------------------------------------------------------------------
   LOGICAL FUNCTION reademptyc0_x( c_emp, ne, ndi )
!-----------------------------------------------------------------------

        ! ...   This subroutine reads empty states from unit emptyunitc0
        ...
        USE mp_wave,            ONLY: splitwf
        USE io_files,           ONLY: outdir, prefix
        USE io_files,           ONLY: empty_file, emptyunitc0
        USE reciprocal_vectors, ONLY: ig_l2g
        USE gvecw,              ONLY: ngw
        !USE control_flags,      ONLY: ndr
        USE xml_io_base,        ONLY: restart_dir, wfc_filename
 ...
 20     FORMAT('*** NGW = ', I8, ' NE = ', I4)
        ...
        RETURN
   END FUNCTION reademptyc0_x


!-----------------------------------------------------------------------
   SUBROUTINE writeemptyc0_x( c_emp, ne, ndi )
!-----------------------------------------------------------------------
        ...
        USE mp_wave,            ONLY: mergewf
        USE mp,                 ONLY: mp_sum
        USE io_files,           ONLY: empty_file, emptyunitc0, outdir, prefix
        USE io_global,          ONLY: ionode, ionode_id, stdout
        USE reciprocal_vectors, ONLY: ig_l2g
        USE gvecw,              ONLY: ngw
        !USE control_flags,      ONLY: ndw

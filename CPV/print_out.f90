...
!=----------------------------------------------------------------------------=!
      ...
      IF( .not. tcg ) THEN
         ...
         if( .not. do_comp ) then
           ...
           WRITE( stdout, 1960 ) nfi, ekinc, temphc, tempp, etot, enthal, econs, &
                            econt, vnhh(3,3), xnhh0(3,3), vnhp(1),  xnhp0(1), &
                            ecomp
         end if
      ELSE
...
1948  FORMAT( I5,1X,F12.2,1X,F6.1,1X,F6.1,4(1X,F11.5),4(1X,F7.4) )
1960  FORMAT( I5,1X,F18.10,1X,F11.1,1X,F6.1,4(1X,F11.5),4(1X,F7.4),1X,F11.5)
...
2949  FORMAT( I6,1X,4(1X,ES18.10), ES18.10 )
      !
      RETURN
   END SUBROUTINE printout_new_real_x
   !  
!=----------------------------------------------------------------------------=!
...
!=----------------------------------------------------------------------------=!
      ...
      REAL(DP)     :: hamn_h, hamn_ah
      REAL(DP)     :: ham_h, ham_ah
!       COMPLEX(DP)     :: hamn_h, hamn_ah
      COMPLEX(DP)     :: ham_h_c, ham_ah_c
      INTEGER      :: i, j, isp, isa, is, ia, kilobytes
      REAL(DP), ALLOCATABLE :: tauw( :, : )
      LOGICAL      :: tsic, tfile, tstdout
      LOGICAL, PARAMETER    :: nice_output_files=.false.
      ...
      IF( ionode ) THEN
         ...
         IF( tprint ) THEN
            ...
            IF ( PRESENT(hamilt) .AND. PRESENT(print_hamilt_norm) ) THEN
			    ...
			    DO j = 1, hamilt(isp)%ydim
			    DO i = 1, hamilt(isp)%xdim
				!
				ham_h   =  0.5d0 * ( hamilt(isp)%rvec(i,j) + hamilt(isp)%rvec(j,i) )
				ham_ah  =  0.5d0 * ( hamilt(isp)%rvec(i,j) - hamilt(isp)%rvec(j,i) )
				!
				hamn_h  =  hamn_h  + DBLE( ham_h  * ham_h )
				hamn_ah =  hamn_ah + DBLE( ham_ah * ham_ah )
				!
			    ...
			    DO i = 1, hamilt(isp)%xdim
				!
				ham_h_c   =  0.5d0 * ( hamilt(isp)%cvec(i,j) + CONJG(hamilt(isp)%cvec(j,i) ))
				ham_ah_c  =  0.5d0 * ( hamilt(isp)%cvec(i,j) - CONJG(hamilt(isp)%cvec(j,i)) )
				!
				hamn_h  =  hamn_h  + ABS( ham_h_c  * ham_h_c ) !warning:giovanni do not know what to put here
				hamn_ah =  hamn_ah + ABS( ham_ah_c * ham_ah_c )!warning:giovanni do not know what to put here
				!
			    ENDDO
			    ENDDO
                        ENDIF
...
1947  FORMAT( 2X,'nfi',4X,'ekinc',2X,'temph',2X,'tempp',8X,'etot',6X,'enthal', &
           & 7X,'econs',7X,'econt',4X,'vnhh',3X,'xnhh0',4X,'vnhp',3X,'xnhp0' )
1948  FORMAT( I5,1X,E12.2,1X,F6.1,1X,F6.1,4(1X,F11.5),4(1X,F7.4) )
1960  FORMAT( I5,1X,ES12.2,1X,F6.1,1X,F6.1,4(1X,F11.5),4(1X,F7.4),1X,F11.5)
2948  FORMAT( I6,1X,ES15.5,1X,ES11.1,1X,ES11.1,4(1X,ES15.5),ES12.2, ES12.2, ES15.5 )
2949  FORMAT( I6,1X,4(1X,ES14.4), ES15.5 )
      !
      RETURN
   END SUBROUTINE printout_new_twin_x
   !
